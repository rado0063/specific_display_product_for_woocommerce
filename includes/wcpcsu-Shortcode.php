...
// get the array of data from the post meta
        ...
        if($atts['post_ids']){
            $common_args['post__in'] = explode(',', $atts['post_ids']);
        }
...
//add filter by category if attribute category not specidied use the current category
        $c_query = get_queried_object();
        if($atts['category_name'])
        {
            $cat = explode(",", $atts['category_name']);
        ...
        }else if ($c_query->slug != "")
        {
            $tax_query             = array(
                array(
                    'taxonomy'      => 'product_cat',
                    'field'         => 'slug',
'terms'         => $c_query->slug, // Possibly 'exclude-from-search' too
'operator'      => 'IN'
)
            );
            $args['tax_query'][] = $tax_query;
...
//EVENEMENT :
                                ...
                                $event=false;
                                foreach ($cat as $categorie_name) {
                                    switch ($categorie_name->slug) {
                                        case 'skiillz-events':
                                        $event=true;
                                        break;
                                    }
                                }
                                $permaliens = preg_replace('#/produit/#i', '/events/',get_the_permalink());
                                $apartir_de =$event ? "hide_apartir" : "show_apartir"; 
                                $apartir_de_booking =($product instanceof WC_Product_Booking) ? "produit_reservable" : "produit_simple";

                                if ($event) {
                                    if ( has_post_thumbnail( $loop->post->ID ) ) { echo '<a href="'.$permaliens.'"><img src="'.esc_url($wpcsu_img).'" class="wpcsp-thumb"  alt="'.get_the_title().'" /></a>'; } else { echo '<a href="'.get_the_permalink().'"><img src="'.wc_placeholder_img_src().'" alt="Placeholder" /><a>'; }
//var_dump(get_the_permalink());
                                ...
                                <section style="display: none">
                                    <div class="lightbox" style="" id="f<?php echo get_the_id();?>">
                                        <div class="row">
                                            <?php
                                            if("yes" == $display_cart) {
                                                if($product instanceof WC_Product_Booking)
                                                {
                                                    $permalien= $event ? $permaliens : $product->get_permalink();
                                                    ?>
                                                    <div class="col-md-4 text-center" style="margin:30px 0 0 0px;">
                                                        <div class="row">
                                                            <?php
                                                        ...
                                                        <div class="row text-center" style="margin-top:15px;width: 100%;">
                                                            <?php $cat = get_the_terms( $product->ID, 'product_cat' );
                                                            foreach ($cat as $categorie_name) {
                                                                switch ($categorie_name->slug) {
                                                                    case 'espace-coworking-a-la-journee':
                                                                    $var=" / jour";
                                                                    break;
                                                                    case 'espace-coworking-a-la-semaine':
                                                                    $var=" / semaine";
                                                                    break;
                                                                    case 'espace-coworking-au-mois':
                                                                    $var=" / mois";
                                                                    break;
                                                                }

                                                            }?>
                                                            <span class="atw_product_price <?php echo $apartir_de_booking." ".$apartir_de; ?>" style="display: block;  margin: 20px 0px 0 0px;font-size: 20px;color:#3DC9AE !important">À partir <?php echo $product->get_price_html().$var;?></span>
                                                        </div>
                                                        <div class="row text-center" style="margin-top:15px;width: 100%;">
                                                            <a href="<?php echo $permalien; ?>" style="color: #3dc9ae !important;border: 2px solid #3dc9ae;border-radius: 5px;padding: 1.5% 2%;font-weight: 600;
                                                            "><i class="fa fa-plus" aria-hidden="true"></i> En savoir plus</a>
                                                        </div>
                                                        <div class="row text-center" style="margin-top:15px;width: 100%;">
                                                            <button class="popup_produit" style="background-color: #b837b2 !important;border:1px solid #b837b2;border-radius: 5px;width: 60%;float: left;margin-left: 20%"><a href="<?php echo $permalien; ?>" style="color: white;font-weight: 600;" class="bookable-custom-button product_type_booking "><i class="fa fa-calendar" aria-hidden="true"></i>  RÉSERVER</a></button>
                                                        </div>
                                                    </div>
                                                    <div class="atw_product_desc col-md-6">
                                                        <h1 style="font-size: 40px; margin-bottom: 30px;color: #bb40b6;font-weight: 600"><?php the_title();?></h1>
                                                        <h2 class="description_popup">Description</h2>
                                                        <?php if ($event){ ?>
                                                            <div class="description_popup_content">
                                                                <?php echo do_shortcode('[description_event post='.$product->ID.']'); ?></div>
                                                                <?php
                                                            }
                                                            else{ ?>
                                                                <div class="description_popup_content">
                                                ...
                                                }else
                                                {
                                                    ...
                                                    <div class="col-md-4 text-center" style="margin:30px 0 0 0px;">
                                                        ...
                                                        <div class="row">
                                                            <span class="atw_product_price <?php echo $apartir_de_booking." ".$apartir_de; ?>" style="display: block;  margin: 20px 0px 0 0px;font-size: 20px;color:#3DC9AE !important">À partir de <?php echo $product->get_price_html(); ?></span>

                                                        </div>
                                                        <div class="row" style="margin:20px 0 0 0px;">
                                                            <a href="<?php echo $product->get_permalink(); ?>" style="color: #3dc9ae !important;border: 2px solid #3dc9ae;border-radius: 5px;padding: 1.5% 2%;font-weight: 600;
                                                            "><i class="fa fa-plus" aria-hidden="true"></i> En savoir plus</a>
                                                ...
                                                <div class="row text-center">
                                                    <div class="col-md-4" style="padding: 6px 0 0 0">
                                                    </div>
                                                    <div class="col-md-8 text-left">
                                                        <div class="row">
                                                            <div class="col-md-4">
                                                                <button class="popup_produit" style="background-color: #009ddc !important;border:1px solid #009ddc;border-radius: 5px;width: 90%;"><a href="?add-to-cart=<?php echo get_the_ID(); ?>" data-quantity="1" class="ajax_add_to_cart" data-product_id="<?php get_the_ID(); ?>" style="color: white;font-weight: 600;"><i class="fa fa-cart-plus fa-lg"></i>  Ajouter au panier</a></button>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <button class="popup_produit" style="background-color: #b837b2 !important;border:1px solid #b837b2;border-radius: 5px;width: 90%"><a href="<?php echo $addtocart_url; ?>&quantity=1&wi_buy_now=1" style="color: white;font-weight: 600;"><i class="fa fa-usd fa-lg"></i>  Acheter maintenant</a></button>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <div class="popup_produit" style="background-color: #fff !important;border:1px solid #009ddc;border-radius: 5px;width: 90%">
                                                                    <?php echo do_shortcode('[button_favorites]'); ?>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                            ...
                            <div class="atw_item_bottom">
                                <?php if ($display_title == "yes") { ?>
                                    <h4 class="atw_item_title">
                                        <?php if ($event) {?>
                                            <a href="<?php echo($permaliens);?>"><?php the_title();?></a>
                                        <?php }else{?>
                                            <a href="<?php the_permalink();?>"><?php the_title();?></a>
                                        <?php } ?>
                                    </h4>
                                ...
                                <div class="atw_item_info">
                                    <?php
                                    if ($display_ratings == "yes") {
                                        echo do_shortcode('[site_reviews_summary assigned_posts="'.get_the_ID().'" hide="bars,rating,summary"]');
                                    ...
                                    if ($display_price == "yes") { ?>
                                        ...
                                        <span class="atw_price showPrice <?php echo $apartir_de_booking." ".$apartir_de; ?>" style="color:#3DC9AE !important">À partir de <?php echo $product->get_price_html().$var; ?></span>  
                                    <?php } ?>
                                    <?php if("yes" == $display_cart) {    
                                        ...
                                        {
                                            $permalien= $event ? $permaliens : $product->get_permalink();  
                                            echo ' <div class="add-cart-bookmark-list">
                                            <ul>
                                            <li style="position:relative" title="Ajouter dans les favories"><i class="favorite_starts fa-xs">&nbsp;</i>
                                            <a onclick="favorite_click(this)" style="cursor:pointer"><i class="fa  fa-heart fa-lg"></i></a>
                                            <span style="display:none">'.do_shortcode('[favorite-post-btn]').'</span></li>
                                            <li title="Acheter maintenant"><a href="'.$permalien.'"><i class="fa fa-usd fa-lg"></i> </a> </li>
                                            <li  title="reserver"><a href="'.$permalien.'" class="bookable-custom-button product_type_booking "><i class="fa fa-calendar fa-lg" aria-hidden="true"></i></a></li>
                                            </ul>
                                            </div>
                                            ';
                                        }else
                                        {
                                            ...
                                            <a onclick="favorite_click(this)" style="cursor:pointer"><i class="fa  fa-heart fa-lg"></i></a>
                                            <span style="display:none">'.do_shortcode('[favorite-post-btn]').'</span></li>
                                            <li  title="Acheter maintenant"><a href="'.$addtocart_url.'&quantity=1&wi_buy_now=1"><i class="fa fa-usd fa-lg"></i> </a> </li>
                                            <li  title="Ajouter au panier"><a href="'.do_shortcode('[add_to_cart_url id="'.$post->ID.'"]').'" data-quantity="1" class="ajax_add_to_cart" data-product_id="'.get_the_ID().'" ><i class="fa fa-cart-plus fa-lg"></i></a></li>
                                            </ul>
                                            </div>
                                            ';
                                        }  
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function()
        {
            jQuery('.wpf-favorite').parents('li').find('i.favorite_starts').removeClass('fa fa-star').addClass('fa fa-star');
            jQuery('.wpf-not-favorite').parents('li').find('i.favorite_starts').removeClass('fa fa-star')
            jQuery(document).ajaxSuccess(function(event,xhr,setting)
            {
                var data      = setting.data;
                var paramsUrl = new URLSearchParams('?' + data);     
                if(paramsUrl.get('action') == 'wfp_action')
                {
                    let res_data_js = jQuery(xhr.responseJSON.data);
                    var className = res_data_js.attr('class');
                    if(className == 'wpf-favorite'){
                        jQuery('.'+className).parents('li').find('i.favorite_starts').removeClass('fa fa-star').addClass('fa fa-star')
                    }else
                    {
                        jQuery('.'+className).parents('li').find('i.favorite_starts').removeClass('fa fa-star')
                    }
                }
            })
        })
                        ...
                        <div class="atw_item atw--single_item atw_card atw_card_rounded">
                            ...
                            <div class="atw_item_bottom">
                                <div class="atw_item_info">
                                    ...
                                    if('yes' == $display_price) {
                                        ?>
                                        <span class="atw_price" style="color:#3DC9AE !important">À partir de <?php echo $product->get_price_html(); ?>
                                    </span>
                                <?php } ?>
                                <?php
                                if ($display_ratings == "yes") {
                                ...
                                <div class="atw_item_bottom">
                                    ...
                                    <div class="atw_item_info">
                                        <?php if('yes' == $display_price) {?>
                                            <span class="atw_price" style="color:#3DC9AE !important"><?php echo $product->get_price_html(); ?></span>
                                        <?php } ?>
                                        <?php
                                            ...
                                            if("yes" == $display_cart) {
                                                if($product instanceof WC_Product_Booking)
                                                {
                                                    ...
                                                    <div class="col-md-4 text-center" style="margin:30px 0 0 0px;">
                                                        ...
                                                        <div class="row text-center" style="margin-top:15px;width: 100%;">
                                                            <button class="popup_produit" style="background-color: #b837b2 !important;border:1px solid #b837b2;border-radius: 5px;width: 60%;float: left;margin-left: 20%"><a href="<?php echo $product->get_permalink(); ?>" style="color: white;font-weight: 600;" class="bookable-custom-button product_type_booking "><i class="fa fa-calendar" aria-hidden="true"></i>  RÉSERVER</a></button>
                                                        </div>
                                                    </div>
                                                    <div class="atw_product_desc col-md-6">
                                                        <h1 style="font-size: 40px; margin-bottom: 30px;color: #bb40b6;font-weight: 600"><?php the_title();?></h1>
                                ...
                                <div class="atw_item_bottom">
                                    ...
                                    <div class="atw_item_info">
                                        ...
                                        <?php if('yes' == $display_price) {?>
                                            ...
                                            <span class="atw_price showPrice" style="color:#3DC9AE !important">À partir de <?php echo $product->get_price_html().$var; ?></span>
                                        <?php } ?>
                                        <?php if("yes" == $display_cart) {
                                            if($product instanceof WC_Product_Booking)
                                            {
                                                ...
                                                <a onclick="favorite_click(this)" style="cursor:pointer"><i class="fa  fa-heart fa-lg"></i></a>
                                                <span style="display:none">'.do_shortcode('[favorite-post-btn]').'</span></li>
                                                <li title="Acheter maintenant"><a href="'.$product->get_permalink().'"><i class="fa fa-usd fa-lg"></i> </a> </li>
                                                <li  title="reserver"><a href="'.$product->get_permalink().'" class="bookable-custom-button product_type_booking "><i class="fa fa-calendar fa-lg"  aria-hidden="true"></i></a></li>
                                                </ul>
                                                </div>
                                                ';
                                            }else
                                            {
                                                ...
                                                <span style="display:none">'.do_shortcode('[favorite-post-btn]').'</span></li>
                                                <li  title="Acheter maintenant"><a href="'.$addtocart_url.'&quantity=1&wi_buy_now=1"><i class="fa fa-usd fa-lg"></i> </a> </li>
                                                <li  title="Ajouter au panier"><a data-quantity="1" class="ajax_add_to_cart" data-product_id="'.get_the_ID().'" href="'.do_shortcode('[add_to_cart_url id="'.$post->ID.'"]').'"><i class="fa fa-cart-plus fa-lg"></i></a></li>
                                                </ul>
                                                </div>
                                                ';
                                            }                                                         
